...
A key design principle: **no copyrighted content from shakespeareswords.com is stored centrally**. The only data retained in any shared, permanent store is either the *absence* of an entry (a single flag indicating a word has no glossary match), an independently authored gloss generated by Claude from its own training, or a bare integer indicating which of several entries best suits a given sentence. The actual definition text, headwords, and scholarly content belonging to shakespeareswords.com are never written to any central database or server-side cache.

As with any website, a user's own browser may cache API responses locally for up to seven days as a performance optimisation — this is standard HTTP caching behaviour, private to that individual's device, and functions no differently to their browser caching any other page they visit. No other party can access this data.
