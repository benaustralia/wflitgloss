  ...
  const h = {
    ...
    speak: async (termOrText = null) => { 
      ...
      if (typeof termOrText === 'string') {
        ...
        termObj = s.terms.find(t => t.term === text);
      ...
      if (!text || s.isGeneratingAudio) return; 
  ...
  return <div className="w-full max-w-md md:max-w-2xl lg:max-w-4xl mx-auto min-h-screen bg-background flex flex-col">
    ...
    {s.view === 'list' ? <div className="flex flex-col h-screen w-full">
        ...
        <div className="flex gap-2 items-center px-4">
          <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input placeholder="Search" value={s.search} onChange={(e) => update({ search: e.target.value })} className="w-full pl-10 h-10 text-sm" onKeyDown={(e) => e.key === 'Enter' && s.search && (!s.terms || !s.terms.length) && h.add(s.search)} /></div>
